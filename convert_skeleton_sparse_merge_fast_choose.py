...
parser.add_argument(
    '-f', '--function', type=str, default='kimimaro',
    choices=['kimimaro', 'skeletor'],
    help='Skeletonization function to use, '
        'options: [kimimaro (Princeton), skeletor (MRC Lab)].'
)
parser.add_argument(
    ...
    if args.function == 'kimimaro':
        ...
        for id in tqdm(range(args.swc_range[0], args.swc_range[1])):
            try:
                ...
                print('WARN: id {:d} is missing, cannot do futher skeletonization.'.format(id))
                continue
            mesh = trimesh.Trimesh(mesh.vertices, mesh.faces)
            mesh_fixed = skeletor.pre.fix_mesh(mesh, remove_disconnected=1000, inplace=False)
            skel = skeletor.skeletonize.by_wavefront(
                mesh_fixed, 
                waves=1, 
                step_size=1
            ) # parameters from skeletor (Wavefront)
            skel_up = Skeleton(segid=id, space='physical')
            skel_up.vertex_types = np.zeros((skel.vertices.shape[0]), dtype=np.uint8)
            skel.vertices[:, 0] *= args.resolution[0]
            skel.vertices[:, 1] *= args.resolution[1]
            skel.vertices[:, 2] *= args.resolution[2]
            skel_up.vertices = skel.vertices.astype(np.float32)
            skel_up.radii = skel.radius[0].astype(np.float32)
            skel_up.edges = skel.edges.astype(np.uint32)
            vol.skeleton.upload(skel_up)
    else:
        print('ERROR: function {:s} is not supported.'.format(args.function))
